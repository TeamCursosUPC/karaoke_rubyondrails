...
class LocalsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post locals_url, params: { local: { descripcion: @local.descripcion, direccion: @local.direccion, email: @local.email, nombre: @local.nombre, telefono: @local.telefono, user_id: @local.user_id } }
    end
  ...
  test "should update local" do
    patch local_url(@local), params: { local: { descripcion: @local.descripcion, direccion: @local.direccion, email: @local.email, nombre: @local.nombre, telefono: @local.telefono, user_id: @local.user_id } }
